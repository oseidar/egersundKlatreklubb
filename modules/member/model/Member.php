...
//print_r($this);
       if($this->memberId == 0 || empty($this->memberId)){
           ...
           try{
                ...
                $sql = "UPDATE  `member` SET firstName = :firstName, lastName = :lastName, bDate = :bDate ,mail= :mail, club = :club , phone = :phone WHERE memberId = :uid LIMIT 1 ;"; 
                $stmt = $dbh->prepare($sql);                
                $stmt->bindParam(':firstName',$this->firstName,PDO::PARAM_STR);
                $stmt->bindParam(':lastName',$this->lastName,PDO::PARAM_STR);
           ...
           try{
                ...
                $row2 = $stmt2->fetch(PDO::FETCH_ASSOC);
                    ...
                    foreach ($member as $key => $value){
                        //echo "$key ". mb_detect_encoding($value,"auto");
                        if(gettype($value) == "string"){
                            if(mb_detect_encoding($value, "auto") == "UTF-8"){
                                echo mb_detect_encoding($value,"auto")."<br>";
                                $set = "set".ucfirst($key);
...
";
                ...
                $stmt = $dbh->prepare($sql);
                $stmt->bindParam(':id',$this->memberId,PDO::PARAM_INT);
                $stmt->bindParam(":year", MemberYear::getCurrentYear(), PDO::PARAM_STR);
